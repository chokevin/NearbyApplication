...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks,
    ...
    private String GENERATED_TOKEN = new BigInteger(130, random).toString(32);

    List<String> NearbyMessageUrls = new ArrayList<>();

    private int ArrayAdapterCounter = 0;

    private Timer mTimer1 = new Timer();
    private TimerTask mTt1;
    private Handler mTimerHandler = new Handler();

    private Timer mTimerUpdate = new Timer();
    private TimerTask mTtUpdate;
    ...
    private SwitchCompat mTokenSwitch;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTokenSwitch = (SwitchCompat) findViewById(R.id.token_switch);
        ...
        mPubMessage = DeviceMessage.newNearbyMessage(getUUID(getSharedPreferences(
                getApplicationContext().getPackageName(), Context.MODE_PRIVATE)), GENERATED_TOKEN);
        ...
        mMessageListener = new MessageListener() {
            ...
            public void onFound(final Message message) {
                ...
                NearbyMessageUrls.add(DeviceMessage.fromNearbyMessage(message).getMessageBody());
                mNearbyDevicesArrayAdapter.remove(NearbyMessageUrls.get(ArrayAdapterCounter));
                if(NearbyMessageUrls.size() > 1) {
                    ArrayAdapterCounter++;
                }
            }
            ...
            public void onLost(final Message message) {
                ...
                mNearbyDevicesArrayAdapter.remove(
                        DeviceMessage.fromNearbyMessage(message).getMessageBody());
                NearbyMessageUrls.remove(DeviceMessage.fromNearbyMessage(message).getMessageBody());
            }
        };
        ...
        mTokenSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    startTimer();
        ...
        if (nearbyDevicesListView != null) {
            ...
            nearbyDevicesListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> adapter, View v, int position,
                                        long arg3) {
                    String MessageURL = (String) adapter.getItemAtPosition(position);
                    // assuming string and if you want to get the value on click of list item
                    // do what you intend to do on click of listview row
                    Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(MessageURL));
        ...
        mTt1 = new TimerTask() {
            public void run() {
                mTimerHandler.post(new Runnable() {
                    public void run() {
                        if (mTokenSwitch.isChecked()) {
                            GENERATED_TOKEN = new BigInteger(130, random).toString(32);
                            ConnectionTask task = new ConnectionTask();
                            String[] params = new String[2];
                            params[0] = "https://ambientdynamix-nearby.mybluemix.net/token_password_url?token=" + GENERATED_TOKEN;
                            task.execute(params);
                            DeviceMessage.fromNearbyMessage(mPubMessage).setMessageBody(GENERATED_TOKEN);
                            mPubMessage = DeviceMessage.newNearbyMessage(getUUID(getSharedPreferences(
                                    getApplicationContext().getPackageName(), Context.MODE_PRIVATE)), GENERATED_TOKEN);
                            unpublish();
                            publish();
    ...
    private void updateTimer() {
        mTtUpdate = new TimerTask() {
            public void run() {
                mTimerUpdateHandler.post(new Runnable() {
                    public void run() {
                        mPubMessage = DeviceMessage.newNearbyMessage(getUUID(getSharedPreferences(
                                getApplicationContext().getPackageName(), Context.MODE_PRIVATE)), GENERATED_TOKEN);
                        unpublish();
        ...
        mTimerUpdate.schedule(mTtUpdate, 1, 4000);
    }
